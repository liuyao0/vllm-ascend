...
#include "rearrange_qkv_gdn_gating_tiling.h"
...
static ge::graphStatus RearrangeQkvGdnGatingTilingFunc(gert::TilingContext* context)
{
    ...
    const auto availableCoreNum = static_cast<uint64_t>(std::min<uint32_t>(aicCoreNum, MAX_AIC_BLOCKS));
    ...
    RearrangeQkvGdnGatingTilingData tilingData;
    ...
    // Block dim is the number of cube cores; KERNEL_TYPE_MIX_AIC_1_2 pairs
    // every cube core with two vector cores for the gating stage.
    context->SetBlockDim(dmaCoreNum);
...
static ge::graphStatus RearrangeQkvGdnGatingTilingParse(gert::TilingParseContext* context)
{
    (void)context;
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(RearrangeQkvGdnGating)
    .Tiling(RearrangeQkvGdnGatingTilingFunc)
    .TilingParse<RearrangeQkvGdnGatingCompileInfo>(RearrangeQkvGdnGatingTilingParse);
}  // namespace optiling
