...
public:
                                ...
                                const RearrangeQkvGdnGatingTilingData& tiling, TPipe* pipe)
    {
        ...
        const uint32_t headRawBytes = AlignUp(heads_ * static_cast<uint32_t>(sizeof(THead)), ALIGN_BYTES);
        const uint32_t headVecBytes = AlignUp(2 * heads_ * FP32_BYTES, ALIGN_BYTES);
    ...
    {
        const uint64_t coreId = static_cast<uint64_t>(GetBlockIdx());
        const uint64_t coreNum = static_cast<uint64_t>(GetBlockNum()) * static_cast<uint64_t>(GetTaskRation());
        if (coreNum == 0 || tokens_ == 0 || heads_ == 0) {
            return;
        }
...
private:
    __aicore__ inline void BuildHeadVectors(uint32_t repeatRows)
    {
        ...
        LocalTensor<float> scaleHead = headVec[heads_];
        LoadHead(raw, dtBias_);
        LoadHead(raw[heads_], aLog_);
        ...
        if constexpr (IsSameType<THead, float>::value) {
            ...
            Adds(scaleHead, raw[heads_], 0.0f, heads_);
        } else {
            Cast(biasHead, raw, RoundMode::CAST_NONE, heads_);
            Cast(scaleHead, raw[heads_], RoundMode::CAST_NONE, heads_);
        }
