...
#include "attention/rearrange_qkv_gdn_gating/rearrange_qkv_gdn_gating_torch_adpt.h"
...
TORCH_LIBRARY_EXPAND(CONCAT(_C, _ascend), ops)
{
    ...
    ops.def(
        "npu_rearrange_qkv_and_gdn_gating(Tensor x, Tensor a, Tensor b, Tensor alog, Tensor dtbias, "
        "int q_dim, int k_dim, int v_dim, float beta=1.0, float threshold=20.0) "
        "-> (Tensor y, Tensor g, Tensor beta_out)"
    );
    ops.impl(
        "npu_rearrange_qkv_and_gdn_gating",
        torch::kPrivateUse1,
        &vllm_ascend::npu_rearrange_qkv_and_gdn_gating);
